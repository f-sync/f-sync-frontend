...
export const AuthenticateSocket = (jwt) => {
    // This emit is to authenticate jwt using a jwt object
    // Use this in the login button
    socket.emit('authenticate', { token: jwt });
}

export const SendLoginEmail = (payload) => {
    // This emit is to send an email with a login link via Twilio
    // Payload object:
    // type: "retail" or "brand"
    // email: company email
    socket.emit("loginEmailRequest", payload)
}
